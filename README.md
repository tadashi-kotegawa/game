...
## 週間ランキング運営サイト (Weekly Ranking Admin)

- `admin.html` を開くと、週間ランキングのルール（表示名・モード・制限時間・ライフなど）を編集できます。
- **かんたん手順**: `admin.html` で「今週を自動セット」→表示名を必要なら変更→「保存」。
- ランキングID（weekKey）は空欄でも保存時に自動生成されます。
- ゲーム本体 (`index.html`) はランキング開始時にこの設定を再取得して反映します。
- 週間設定ロジックは `weekly-settings.js` に共通化しており、ゲーム/管理画面で同じルールを参照します。
- 本番公開時は、**adminページを別ドメイン/別サイトに分離**し、Firebase Security Rulesで管理者だけが更新できるようにしてください。
